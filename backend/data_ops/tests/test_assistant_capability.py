...
from data_ops.assistant import assistant_provider, get_assistant_capability


def test_data_ops_registers_tools_and_local_skills():
    capability = get_assistant_capability()

    assert isinstance(assistant_provider, AssistantCapabilityProvider)
    assert capability.app_key == "data_ops"
    assert capability.required_feature == "data_ops"
    assert {tool.name for tool in capability.tools} == {
        "data_ops_get_schema",
