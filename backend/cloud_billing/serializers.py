...
class CloudProviderSerializer(serializers.ModelSerializer):
    ...
    def validate_recharge_info(self, value):
        """
        Validate and normalize recharge info into canonical JSON text.
        """
        text = str(value or "").strip()
        if not text:
            return ""

        provider = getattr(self, "instance", None)
        if provider is None:
            return text
...
class RechargeApprovalRecordSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'triggered_by_username_snapshot', 'submitted_by',
            'submitted_by_username', 'submitter_identifier',
            'resolved_submitter_user_id', 'submitter_user_label',
