...
def test_validate_recharge_request_payload_requires_cloud_type():
...
def test_prepare_recharge_request_payload_injects_cloud_type():
    ...
    assert payload.get("amount") in {"", None}


def test_validate_recharge_request_payload_allows_missing_amount():
    ...
    assert payload["cloud_type"] == "阿里云"
    assert payload["currency"] == "CNY"
    assert payload.get("amount") in {"", None}


def test_validate_recharge_request_payload_requires_payee_fields():
...
def test_recharge_approval_script_pages_history_in_ten_item_batches_and_skips_deleted_instances(monkeypatch):
    ...
    second_page_codes = ["ins_new_match"]
    ...
    def fake_get_instance(*args, **kwargs):
        ...
        if instance_code == "ins_old_match":
            return {
                "data": {
        ...
        if instance_code == "ins_new_match":
            return {
                "data": {
    ...
    assert result["found"] is True
    assert result["source_instance"]["instance_code"] == "ins_new_match"
...
@pytest.mark.django_db
def test_check_ongoing_recharge_approval_submission_uses_local_record_when_instance_code_missing(
    cloud_provider,
    monkeypatch,
):
    cloud_provider.recharge_info = '{"amount": 188, "recharge_account": "acct-188"}'
...
def test_submit_recharge_skill_prefers_latest_account_state():
    module = _load_skill_module(
        "submit_recharge_approval_skill_recent_first",
        "scripts/submit_recharge_approval.py",
    )

    def fake_list_instances(*args, **kwargs):
        return {"data": {"instance_code_list": ["ins_old_pending", "ins_new_finished"]}}

    def fake_get_instance(*args, **kwargs):
        instance_code = args[3]
        if instance_code == "ins_old_pending":
            return {
                "data": {
                    "status": "PENDING",
                    "serial_number": "SN-old",
    ...
    assert out["state"] == "finished"
    assert out["instance_code"] == "ins_new_finished"
    assert out["start_time"] == "2025-01-02T00:00:00Z"
