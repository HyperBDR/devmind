...
class CloudProvider(models.Model):
    ...
    provider_type = models.CharField(
        max_length=20, choices=PROVIDER_TYPES, help_text="Cloud provider type"
    )
    ...
    config = models.JSONField(
        help_text=(
            ...
            "notifications, set config['notification'] = "
            "{type: 'webhook'|'email', channel_uuid: '<uuid>', "
            "email_to: ['a@b.com', ...]} (email_to is an array of "
            "strings, used when type is 'email')."
        )
    )
...
class BillingData(models.Model):
    ...
    provider = models.ForeignKey(
        CloudProvider, on_delete=models.CASCADE, related_name="billing_data"
    )
    ...
    hour = models.IntegerField(
        choices=HOUR_CHOICES, help_text="Collection hour, 0-23"
    )
    ...
    service_costs = models.JSONField(
        default=dict,
        help_text=(
            "Service cost breakdown, format: " "{'service_name': cost, ...}"
        ),
    )
...
class AlertRecord(models.Model):
    ...
    provider = models.ForeignKey(
        CloudProvider, on_delete=models.CASCADE, related_name="alert_records"
    )
    ...
    previous_cost = models.DecimalField(
        max_digits=20, decimal_places=2, help_text="Previous hour cost"
    )
    ...
    webhook_response = models.JSONField(
        null=True, blank=True, help_text="Webhook response content"
    )
    webhook_error = models.TextField(
        blank=True, help_text="Webhook error message if failed"
    )
