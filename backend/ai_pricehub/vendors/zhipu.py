...
def sync_vendor_catalog() -> dict:
    ...
    priced_models = [
        ...
        if item["family"] == "text"
        and (
            item["input_price_per_million"] is not None
            or item["output_price_per_million"] is not None
        )
    ]
    return {
        "vendor": {
...
def _pricing_score(item: dict) -> tuple[int, float]:
    ...
    total = float(item["input_price_per_million"] or 0) + float(
