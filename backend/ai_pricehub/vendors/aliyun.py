...
def sync_vendor_catalog() -> dict:
    ...
    return {
        ...
        "models": models,
        "notes": f"Extracted {len(models)} priced models from the official Aliyun Model Studio pricing page.",
    }
...
def _extract_models(html: str) -> list[dict]:
    ...
    for row in rows:
        ...
        item = {
            ...
            "_market_scope": market_scope,
        }
        _upsert_model(models, item)
    cleaned = []
    for item in sorted(models.values(), key=lambda model: model["model_name"].lower()):
...
def _find_mode_text(texts: list[str]) -> str | None:
    return next(
        (text for text in texts if "思考模式" in text or "非思考" in text),
        None,
    )
...
def _is_section_heading(texts: list[str]) -> bool:
    ...
    if any(token in merged for token in ["计费规则", "影响计费", "说明", "更多模型"]):
        return True
    return bool(re.fullmatch(r"[\u4e00-\u9fffA-Za-z0-9 ._-]{2,40}", merged)) and not lowered.startswith(
        ("qwen", "deepseek", "qwq", "qvq", "glm", "kimi", "moonshot-kimi")
    )
...
def _detect_model_name(texts: list[str]) -> str | None:
    for text in texts:
        ...
        if any(token in text for token in ["模式", "半价", "享有折扣", "免费额度"]):
            continue
        if lowered.startswith(("qwen", "deepseek", "qwq", "qvq", "glm", "kimi", "moonshot-kimi")):
            return text
    return None


def _derive_family_from_name(name: str) -> str | None:
    return "text" if name.lower().startswith(("qwen", "deepseek", "qwq", "glm", "kimi", "moonshot-kimi")) else None
...
def _resolve_pricing(
    *,
    ...
    if range_text:
        return prices[0], prices[1], f"Aliyun official pricing row for input range {range_text}."
    ...
    return input_price, output_price, f"Aliyun official pricing row for mode {mode_text or 'unknown mode'}."
...
def _prefer_candidate(existing: dict, candidate: dict) -> bool:
    ...
    existing_market_scope = existing.get("_market_scope", "unknown")
    candidate_market_scope = candidate.get("_market_scope", "unknown")
    existing_total = float(existing.get("input_price_per_million") or 0) + float(
        existing.get("output_price_per_million") or 0
    )
    candidate_total = float(candidate.get("input_price_per_million") or 0) + float(
        candidate.get("output_price_per_million") or 0
    )
    if candidate_market_scope != existing_market_scope:
        if candidate_market_scope == "domestic":
            return True
        if existing_market_scope == "domestic":
            return False
    ...
    if candidate_total != existing_total:
        return candidate_total > existing_total
    existing_range = existing.get("notes", "")
    candidate_range = candidate.get("notes", "")
    return len(candidate_range) > len(existing_range)
