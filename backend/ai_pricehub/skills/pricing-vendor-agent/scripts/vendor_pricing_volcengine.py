...
def _extract_zones(html: str) -> dict[str, str]:
    pattern = re.compile(r'\\"ops\\":\[(.*?)\],\\"zoneId\\":\\"([^\\"]+)\\",\\"zoneType\\":\\"Z\\"', flags=re.DOTALL)
    zones = {}
...
def _extract_rows(*, zones: dict[str, str], schema: dict[str, str], section_name: str) -> list[dict]:
    prefixes = sorted({zone_id[:-SUFFIX_LEN] for zone_id in zones if zone_id.endswith(schema["model"]) and zones.get(zone_id, "").strip()})
    rows = []
...
def _candidate_score(item: dict) -> tuple[int, int, float]:
    ...
    total = float(item.get("input_price_per_million") or 0) + float(item.get("output_price_per_million") or 0)
