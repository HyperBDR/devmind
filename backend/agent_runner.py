...
class LangfuseAgentCallbackHandler(BaseCallbackHandler):
    """Small LangChain-core callback that writes traces without langchain package dependency."""
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        self.inputs[key] = _extract_message_text(messages)
    ...
    def on_llm_end(
        ...
        **kwargs: Any,
        ...
        self.client.generation(
            ...
            name="chat_model",
            start_time=self.starts.get(key),
...
# ---------------------------------------------------------------------------
# SkillSpec — definition for one skill sub-agent
# ---------------------------------------------------------------------------
...
class AgentRunner(ABC):
        ...
        *,
        ...
        if workspace_root:
            ...
            if env_root:
                ...
                # parents[1] of agent_runner.py → project root (e.g. /opt/backend/)
    ...
    def execute(self) -> Dict[str, Any]:
        ...
        callbacks = [self._build_callback_handler()]
        langfuse_handler = self.get_langfuse_callback_handler()
            ...
            for key in ("output", "final", "response"):
                ...
                if isinstance(candidate, str):
                    ...
                    if parsed is not None:
                        ...
                        break
            if structured is None:
                parsed = _parse_json_object(_extract_message_text(result.get("messages")))
                if parsed is not None:
                    structured = parsed
        if structured is None:
            structured = self.recover_structured_response(result)
        if structured is None:
            raw_result_text = _extract_message_text(result)
            try:
    ...
    def _build_callback_handler(self) -> Any:
    ...
    def get_langfuse_callback_handler(self) -> Any:
        ...
        if not LANGFUSE_ENABLED or not LANGFUSE_PUBLIC_KEY or not LANGFUSE_SECRET_KEY:
            logger.info(
                "Langfuse tracing disabled by configuration "
                "(enabled=%s, public_key=%s, secret_key=%s).",
                LANGFUSE_ENABLED,
        ...
        if self.record:
            metadata["record_id"] = getattr(self.record, "id", None) or str(self.record)
            metadata["record_trace_id"] = str(getattr(self.record, "trace_id", "") or "")

        tags = ["agent", "langgraph"]
        name = f"{self.__class__.__name__}"
        trace_id = (
            str(getattr(self.record, "trace_id", "") or "")
            or langfuse_client.get_trace_id()
        )

        return LangfuseAgentCallbackHandler(
