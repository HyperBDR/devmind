...
def same_cloud_recharge_account(form_map: dict[str, Any], expected: dict[str, Any]) -> bool:
    return (
        form_map.get(FIELD_CLOUD_TYPE) == expected[FIELD_CLOUD_TYPE]
        and form_map.get(FIELD_RECHARGE_ACCOUNT) == expected[FIELD_RECHARGE_ACCOUNT]
    )
...
def inspect_recharge_account_state(
    ...
    """
    Inspect live Feishu instances for the request recharge account.

    Returns a normalized state summary that higher layers can use to decide
    whether to reuse an ongoing instance or proceed with a new submission.
    """
    expected = expected_name_map(request_data)
    now = dt.datetime.now(dt.timezone.utc)
    end_time_ms = int(now.timestamp() * 1000)
    start_time_ms = int(
        (now - dt.timedelta(days=lookback_days)).timestamp() * 1000)
    listed = list_instances(
        base_url, token, approval_code, start_time_ms, end_time_ms, limit=100
    )
    ...
    if finished_matches:
        ...
        return {
            ...
            "cloud_type": expected[FIELD_CLOUD_TYPE],
            "recharge_account": expected[FIELD_RECHARGE_ACCOUNT],
            "instance_code": latest["instance_code"],
