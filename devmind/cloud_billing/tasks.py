...
def submit_recharge_approval(
    ...
    *,
    ...
    if ongoing_check["blocked"]:
        ...
        log_collector.error(
            "Recharge account already has an ongoing recharge approval; "
            f"blocking duplicate submission (instance_code={existing_payload['instance_code']})"
        )
        if task_id:
            TaskTracker.update_task_status(
                task_id=task_id,
                ...
                metadata=task_log_metadata(
                    ...
                    progress_message="Recharge account already has an ongoing approval.",
    ...
    if account_state and account_state.get("state") == "finished":
        ...
        if finished_record is not None and finished_record.status != finished_status:
            finished_record.status = finished_status
            finished_record.status_message = str(
                account_state.get("status") or account_state.get("live_status") or ""
            )
            finished_record.latest_stage = "status_check"
            finished_record.save(
