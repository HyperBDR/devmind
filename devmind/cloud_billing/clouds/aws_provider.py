...
class AWSCloud(BaseCloudProvider):
    ...
    def __init__(self, config: AWSConfig):
        ...
        logger.info(
            f"Initialized AWS Cloud provider with config: {sanitized_config}"
        )
    ...
    def sts_client(self):
        ...
        if self._sts_client is None:
            # Configure retry behavior: reduce retries for validation
            # SSL errors and connection errors should fail fast
            # Only try once for validation
    ...
    def validate_credentials(self) -> bool:
        ...
        Raises:
            Exception: If validation fails, raises exception with error details
        """
        try:
            ...
            logger.warning(
                f"Failed to validate AWS credentials: "
                f"{error_code} - {error_message}"
            )
            # Raise exception with error code for better error handling
            raise Exception(f"{error_code}: {error_message}")
        except Exception as e:
            error_msg = str(e)
            logger.warning(f"Failed to validate AWS credentials: {error_msg}")
            # Re-raise to allow service layer to handle it
            raise
