...
def _to_int(value, default=0):
    try:
        v = int(value)
        return max(v, 0)  # ensure non-negative to satisfy DB check constraint
    except Exception:
        return default
...
def _to_decimal(value, default="0"):
    try:
        v = Decimal(str(value))
        return max(v, Decimal("0"))  # ensure non-negative to satisfy DB check constraint
    except Exception:
...
def collect_data_source(data_source_id, task_id=None, trigger_mode="manual", celery_task_id=""):
    ...
    print(f"[HyperBDR Collect] DataSource: name={source.name} api_url={source.api_url} username={source.username} password={source.password}", file=sys.stderr)
    logger.warning(
