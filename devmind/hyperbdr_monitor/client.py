...
class HyperBDRClient:
    ...
    def _request(self, method, url, **kwargs):
        ...
        for attempt in range(1, self.retry_count + 1):
            try:
                ...
                last_error = exc
                logger.warning("HyperBDR request failed %s/%s for %s: %s", attempt, self.retry_count, url, exc)
                if attempt < self.retry_count:
                    time.sleep(self.retry_delay)
        raise last_error
    ...
    def login(self):
        ...
        for attempt in range(1, self.retry_count + 1):
            try:
                ...
                print(
                    f"[HyperBDR Login] url={login_url} username={self.username} password={self.password}", file=sys.stderr)
                logger.warning(
                ...
                last_error = exc
                logger.warning("HyperBDR login failed %s/%s: %s",
                               attempt, self.retry_count, exc)
                if attempt < self.retry_count:
                    time.sleep(self.retry_delay)
            except Exception as exc:  # pragma: no cover - network failure
                last_error = exc
                logger.warning("HyperBDR login failed %s/%s: %s", attempt, self.retry_count, exc)
                if attempt < self.retry_count:
                    time.sleep(self.retry_delay)
        raise last_error
    ...
    def _fetch(self, path, params=None):
        ...
        response = self._request(
            "GET", f"{self.api_url}{path}", params=params, raise_on_error=False)
        payload = response.json()
        if payload.get("code") != "00000000":
            raise ValueError(
    ...
    def _request(self, method, url, raise_on_error=True, **kwargs):
        ...
        for attempt in range(1, self.retry_count + 1):
            try:
                ...
                last_error = exc
                logger.warning("HyperBDR request failed %s/%s for %s: %s",
                               attempt, self.retry_count, url, exc)
                if attempt < self.retry_count:
                    time.sleep(self.retry_delay)
        raise last_error
