...
from .llm_config import resolve_parser_llm_settings
...
def build_tracking_state(
    *,
    ...
    vendor = vendor or {}
    tracking_context = dict(vendor.get("_tracking_context") or {})
    state = {
        "node_name": node_name,
        "source_type": tracking_context.get("source_type") or "ai_pricehub",
        "source_task_id": tracking_context.get("source_task_id") or "",
...
def invoke_tracked_structured_llm(
    *,
    ...
    preferred_config_uuid: str | None,
    node_name: str,
    ...
    if config_uuid:
        ...
        provider = str(llm_settings.get("provider") or "openai_compatible").strip().lower()
        config = {
