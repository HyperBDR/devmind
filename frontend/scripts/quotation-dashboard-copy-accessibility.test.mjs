...
test('dashboard uses quotation-only overview copy and stable recent fields', () => {
  ...
  assert.equal(copy.rangeSummaryTitle, 'Quotes in selected range')
  assert.equal(copy.recentOverviewTitle, 'Recently updated quotes')
  assert.match(dashboard, /id="dashboard-month-summary"/)
  assert.match(dashboard, /data-testid="dashboard-date-from"/)
  assert.match(dashboard, /data-testid="dashboard-date-to"/)
  assert.match(dashboard, /selectedDateFrom/)
  assert.match(dashboard, /selectedDateTo/)
  assert.match(dashboard, /getDashboardSummary\(/)
  assert.match(dashboard, /dateFrom/)
  assert.match(dashboard, /dateTo/)
  assert.match(dashboard, /getDashboardAnalytics\(currency, dateFrom, dateTo\)/)
  assert.match(dashboard, /function openSelectedRangeQuotes/)
  assert.match(dashboard, /createdFrom/)
  assert.match(dashboard, /createdTo/)
  assert.match(dashboard, /tab: 'list'/)
  assert.match(dashboard, /openSelectedRangeQuotes/)
  assert.match(dashboard, /summary\?\.monthQuoteCount/)
  assert.match(dashboard, /monthQuoteDeltaLabel/)
  assert.match(dashboard, /previousPeriodCount/)
