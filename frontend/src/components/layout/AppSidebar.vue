  ...
  >
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto flex flex-col">
      <div class="flex-1 space-y-1">
        ...
        <!-- SALs Dashboard -->
        <router-link
          v-if="userStore.userHasFeature('sales_work_orders')"
          :to="{ name: 'SalesDashboard' }"
          class="nav-item"
          :class="isActive('/sals/dashboard') ? 'nav-item-active' : ''"
          @mouseenter="preloadRoute('/sals/dashboard')"
        >
          <svg
            class="w-5 h-5"
            fill="none"
          ...
          <span>{{ t('sals.dashboard.title') }}</span>
        </router-link>
